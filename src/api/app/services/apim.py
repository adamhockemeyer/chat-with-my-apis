...
def fetch_apis_by_product(access_token, product_id):
    ...
    url = f'{azure_mgmt_apim_base_url}/products/{product_id}/apis?api-version={get_settings().azure_apim_service_api_version}'
...
def fetch_openapi_spec(api_id, access_token):
    ...
    url = f'{azure_mgmt_apim_base_url}/apis/{api_id}?export=true&format=openapi&api-version={get_settings().azure_apim_service_api_version}'
    response = requests.get(url,
                            headers=headers,
                            timeout=30)
    response.raise_for_status()
